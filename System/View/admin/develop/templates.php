...
<script type="text/javascript">
    ...
    $(function(){
        $('.table_btn_Install').click(function(){
            TemplatesId = $(this).attr('data-id');
            $.get('/admin/api/templatesGetVerion', {'TemplatesId':TemplatesId}, function(Res){
                if(Res.Code != 0){
                    alert(Res.Msg); return false;
                }
                let DownList = Res.Data;
                $('#DownDevModel').find('.modal-title').html('模版版本选择');
        ...
        $('#DevInstallBtn').click(function(){
            if(!confirm("确定安装此模版？")) return;
                ...
                $.get('/admin/api/templateInstall', {'TemplatesId':TemplatesId, 'Version':Version, 'IsDev':'1'}, function(Res){
                    if(Res.Code != 0){
