...
<span>
                                            ...
                                            <div >
                                                    ...
                                                    <div class="file-name">
                                                        ...
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            ...
                                                            <span>
                                                                <?
                                                                if(in_array($v['PluginId'], $PaidIDs)){
                                                                    if(isset($TempFolder[$v['NameKey']])){
    ...
    <script type="text/javascript">
        ...
        $(function(){
            ...
            $('.tempViewBtn').click(function(){ // 插件没有演示可以查看
                /*SelectIndex = $(this).attr('data-index');
                let NameKey = PluginArr[SelectIndex]['NameKey'];
                $('#tempViewModal .modal-title').html(PluginArr[SelectIndex]['Name']);
                $('#tempViewModal .modal-body').html(`
                    <img class="img-fluid" src="`+PluginArr[SelectIndex]['Pic']+`"/>
                `);
            ...
            $('.InstallBtn').click(function(){ // 安装
                ...
                let NameKey = PluginArr[Index]['NameKey'];
                if(typeof TempFolder[NameKey] != 'undefined'){
                    alert('已安装，请先删除，再安装');return;
                }
                $('#LoadingModel').find('.Content').html('安装插件中');
                $('#LoadingModel').modal();
                $.get('/admin/api/pluginInstall', {PluginId:PluginArr[Index]['PluginId'], Version:PluginArr[Index]['LastVersion']}, function(Res){
                    if(Res.Code != 0){
            ...
            $('.BuyBtn').click(function(){
                let Index = $(this).attr('data-index');
                $.get('/admin/api/payplugin', {PluginId:PluginArr[Index]['PluginId']}, function(Res){
                    if(Res.Code != 0){
                        alert(Res.Msg);return;
                    }
