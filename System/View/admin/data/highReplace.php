...
<script type="text/javascript">
    $(function(){
        fillFiled();
        $('#Input_TableName').change(fillFiled);
    })

    var fillFiled = function(){
        $.post('/admin/api/tableField', {'TableName':$('#Input_TableName').val()}, function(Res){
            if(Res.Code != 0){
                alert(Res.Msg);return;
            }
            $('#Input_Field').html('');
            for(let Index in Res.Data){
                let Rs = Res.Data[Index];
                $('#Input_Field').append('<option value="'+Rs.Field+'">'+Rs.Field+'</option>');
            }
        }, 'json')
    }
</script>
